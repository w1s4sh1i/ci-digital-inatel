...
module i2c_master_tb;
	...
	localparam WIDTH = 8; 
    reg clk, reset, start, ack_master, rw;
    reg	[WIDTH-1 : 0] data_in;
	wire [WIDTH-1 : 0] data_slave;
    tri1 scl, sda; // Linha tri-state
    wire done;
    reg [WIDTH-2 : 0] slave_addr;
	 
    // Variaveis internas para capturar os dados recebidos e o endereco
    reg [WIDTH-2 : 0] received_addr;   // Registrador para armazenar o endereco recebido
    reg [WIDTH-1 : 0] received_data;   // Registrador para armazenar os dados recebidos
    
    reg [3:0]	addr_bit_counter, // Contador para os bits do endereco
    			data_bit_counter; // Contador para os bits dos dados

    // Variavel para simular o escravo
    reg sda_slave;

	reg [WIDTH-1 : 0] dado_escravo;

	integer i, negedge_detected;
    ...
    always @(posedge scl) begin
		if (uut.state == 2) begin
            if (addr_bit_counter < 7) begin
            	// Armazena o bit do endereco recebido
                received_addr[6 - addr_bit_counter] <= sda;
                // Incrementa o contador de bits do endereco 
