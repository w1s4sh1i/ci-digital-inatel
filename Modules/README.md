# CI DIGITAL - INATEL (T2/25): Catálogo dos Módulos

> Mapeamento das disciplinas, descrição e orientadores.

|CID-SD   	|MODULE                                      					|LEVEL		 |FASE |ADVISOR				| :white_square_button: |
|:----: 	|:----                                          				|:----		 |:---:|:---:               | :----:               	|
|SD101		|[Introdução à Microeletrônica](/CID-BASIC/SD100-INTRO-MICROEL)	|BASIC		 |1	   |ROCHA, Felipe		|  :white_check_mark:	|
|SD112  	|[Introdução a Verilog](/CID-BASIC/SD112-INTRO-VERILOG)         |BASIC		 |1	   |					|  :white_check_mark:   |
|SD122  	|[Circuitos Digitais I](/CID-BASIC/SD122-CIRC-DIGITAIS-I)       |BASIC		 |1	   | 		            |  :white_check_mark:   |
|SD132 		|[Circuitos Digitais II](/CID-BASIC/SD122-CIRC-DIGITAIS-II)     |BASIC		 |1	   | 		            |  :white_check_mark:   |
|SDXXX 		|[VHDL](/CID-BASIC/VHDL)                                        |BASIC		 |1	   | 		            |  :white_check_mark:   |
|SD142 		|[Circuitos Digitais III](/CID-BASIC/SD122-CIRC-DIGITAIS-III)   |BASIC		 |1	   | 		            |  :white_check_mark:   |
|SD192		|[Projeto Orientado I](/CID-BASIC/SD-192-Trabalho-Orientado-I)  |BASIC		 |1	   | 		            |  :white_check_mark:   |
|SD242 		|[Verificação (UVM)](/CID-ADVANCED/SD242-Verificacao-UVM)       |ADVANCED	 |2	   |PEREIRA, Elivander	|  :white_square_button:|
|SD212 		|[Arquitetura de Sistemas Digitais](/CID-ADVANCED/)             |ADVANCED	 |2	   | 		            |  :white_square_button:|
|SD221 		|[](/CID-ADVANCED/)                                             |ADVANCED	 |2	   | 		            |  :white_square_button:|
|SD232 		|[](/CID-ADVANCED/)                                             |ADVANCED	 |2	   | 		            |  :white_square_button:|
|SDYYY 		|[](/CID-ADVANCED/)                                             |ADVANCED	 |2	   | 		            |  :white_square_button:|
|SDZZZ 		|[](/CID-RECIDENCY-or-PROJECT/)                                 |RESIDENCY or PROJECT |3| 				|  :white_square_button:|

## Análise do programa

> ;

## Descrição das Etapas

1. Básica - : 

2. Avançada - :

3. Projeto ou Residência - : 

## Certificações dos Módulos

> **Básico**: []();

> **Avançado**: []();

> **Projeto Final (conclusão)**: []();

## Anotações

> 

