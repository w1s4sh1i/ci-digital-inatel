...
module axi4_lite_system_tb;
    ...
    task check_result;
        ...
        begin
            ...
            // realizar comparativo e 
            if (expected === actual) begin
                $display("[OK] %4s;", test_msg);
            end else begin
                $display("[ERRO] %4s | Esperado: 0x%08x, Obtido: 0x%08x;", test_msg, expected, actual);
            end
            
            // Retornar o valid === (expected === actual);
            // Realizar contagem em outra task; 
            testes_passivos = testes_passivos + ( valid === (expected === actual) );
            testes_negativos = testes_negativos + ( valid !== (expected === actual) );
        end
    endtask
    ...
    initial begin
        ...
        check_result(1, 32'h00000011, DUT_system.UUT_subordinate.reg_s[0], "Teste 2: Escrita bem sucedida no reg0 (0x00)");

        axi_write(32'h00000004, 32'h00000022);
        check_result(1, 32'h00000022, DUT_system.UUT_subordinate.reg_s[1], "Teste 3: Escrita bem sucedida no reg1 (0x04)");

        // TESTES 4 e 5: Injeção Direta (Simulação de hardware externo)
        DUT_system.UUT_subordinate.reg_s[2] = 32'h00000033;
        #1;
        check_result(1, 32'h00000033, DUT_system.UUT_subordinate.reg_s[2], "Teste 4: Injetado valor externo em reg2_din");

        DUT_system.UUT_subordinate.reg_s[3] = 32'h00000044;
        #1;
        check_result(1, 32'h00000044, DUT_system.UUT_subordinate.reg_s[3], "Teste 5: Injetado valor externo em reg3_din");

        // TESTES 6 a 9: Leituras via Master
        axi_read(32'h00000000, read_val);
        check_result(1, 32'h00000011, read_val, "Teste 6: Leitura do reg0 (0x00)");

        axi_read(32'h00000004, read_val);
        check_result(1, 32'h00000022, read_val, "Teste 7: Leitura do reg1 (0x04)");

        axi_read(32'h00000008, read_val);
        check_result(1, 32'h00000033, read_val, "Teste 8: Leitura do reg2 (0x08 - Externo)");

        axi_read(32'h0000000C, read_val);
        check_result(1, 32'h00000044, read_val, "Teste 9: Leitura do reg3 (0x0C - Externo)");
        ...
        check_result(0, 32'h00000033, DUT_system.UUT_subordinate.reg_s[2], "Teste 11: Escrita rejeitada em reg2 (Read-Only)");

        axi_write(32'h0000000C, 32'hAAAAFFFF);
        check_result(0, 32'h00000044, DUT_system.UUT_subordinate.reg_s[3], "Teste 12: Escrita rejeitada em reg3 (Read-Only)");
